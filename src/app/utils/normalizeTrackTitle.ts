...
function deduplicateArtist(title: string, artist: string): string {
  ...
  // Cleanup separators and orphaned parentheses
  result = result
    .replace(/\(\s+/g, '(').replace(/\s+\)/g, ')')  // ( text ) → (text)
    .replace(/^[\s\-–—|:,]+/, '')
    .replace(/[\s\-–—|:,]+$/, '')
    .replace(/\s*\([^)]*$/, '')    // unclosed ( with no matching )
    .replace(/\s{2,}/g, ' ')
    .trim();
...
function stripSymbols(str: string): string {
  let r = str;
  r = r.replace(/\p{Extended_Pictographic}[\uFE0E\uFE0F]?/gu, ''); // emojis
  r = r.replace(/([!?]){2,}/g, '$1');          // !!! → !, ??? → ?
