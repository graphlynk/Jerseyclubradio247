...
export const TrackCard = React.memo(function TrackCard({ track, trackList, index, variant = 'grid' }: TrackCardProps) {
  ...
  if (variant === 'list') {
    return (
      ...
      >
        {index !== undefined && (
          <span className={`text-sm w-5 text-center font-mono ${isActive ? 'text-[#9D00FF]' : 'text-[#7B6F90]'}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[#b8b0cc] font-medium truncate max-w-[calc(100vw-190px)] md:max-w-none">{formatArtistName(track.snippet.channelTitle)}</p>
        </div>
        {/* Crate button (list) */}
        <button
          onClick={handleCrate}
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        ...
        <p className="text-xs text-[#b8b0cc] font-medium">{formatArtistName(track.snippet.channelTitle)}</p>
      </div>
    </div>
  );
});